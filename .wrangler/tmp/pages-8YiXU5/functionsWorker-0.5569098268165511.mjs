...
async function onRequest(context) {
  ...
  try {
    ...
    if (existing) {
      ...
      user = await env.DB.prepare("SELECT * FROM users WHERE id = ?").bind(existing.id).first();
    } else {
...
async function onRequest2(context) {
  ...
  try {
    ...
    const imagePrompts = parsedModule.image_prompts || [];
...
async function onRequest5(context) {
  ...
  try {
    ...
    return errorResponse(`Failed to fetch text models: ${err instanceof Error ? err.message : "Unknown"}`, env, 500);
  }
}
__name(onRequest5, "onRequest");

// ../.wrangler/tmp/pages-8YiXU5/functionsRoutes-0.8698971254945925.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-Lar5tB/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-Lar5tB/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.5569098268165511.mjs.map
